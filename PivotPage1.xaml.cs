...
namespace sdkRSSReaderCS
{
    public partial class PivotPage1 : PhoneApplicationPage
    {
        ...
        private void loadFeedButton_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            ...
            webClient.DownloadStringAsync(new System.Uri("http://feeds.feedburner.com/TechCrunch/startups"));
        }

        // Click handler which runs when the 'Load Feed' or 'Refresh Feed' button is clicked. 
        private void loadFeedButton_Click2(object sender, System.Windows.RoutedEventArgs e)
        {
            // WebClient is used instead of HttpWebRequest in this code sample because 
            // the implementation is simpler and easier to use, and we do not need to use 
            // advanced functionality that HttpWebRequest provides, such as the ability to send headers.
            ...
            // Download the RSS feed. DownloadStringAsync was used instead of OpenStreamAsync because we do not need 
            // to leave a stream open, and we will not need to worry about closing the channel. 
            webClient.DownloadStringAsync(new System.Uri("http://feeds.feedburner.com/hacker-news-feed-200?format=xml"));
        }

        // Click handler which runs when the 'Load Feed' or 'Refresh Feed' button is clicked. 
        private void loadFeedButton_Click3(object sender, System.Windows.RoutedEventArgs e)
        {
            // WebClient is used instead of HttpWebRequest in this code sample because 
            // the implementation is simpler and easier to use, and we do not need to use 
            // advanced functionality that HttpWebRequest provides, such as the ability to send headers.
            ...
            // Subscribe to the DownloadStringCompleted event prior to downloading the RSS feed.
            webClient.DownloadStringCompleted += new DownloadStringCompletedEventHandler(webClient_DownloadStringCompleted3);

            // Download the RSS feed. DownloadStringAsync was used instead of OpenStreamAsync because we do not need 
            // to leave a stream open, and we will not need to worry about closing the channel. 
            webClient.DownloadStringAsync(new System.Uri("http://blogs.technet.com/b/microsoft_blog/rss.aspx"));
        }
        ...
        // Event handler which runs after the feed is fully downloaded.
        private void webClient_DownloadStringCompleted3(object sender, DownloadStringCompletedEventArgs e)
        {
            if (e.Error != null)
            {
                Deployment.Current.Dispatcher.BeginInvoke(() =>
        ...
        protected override void OnNavigatedTo(System.Windows.Navigation.NavigationEventArgs e)
        {
            ...
            if (this.State.ContainsKey("feed"))
            {
                ...
                {
                    UpdateFeedList2(State["feed"] as string);
                }
                if (feedListBox3.Items.Count == 0)
                {
        ...
        // This method sets up the feed and binds it to our ListBox. 
        private void UpdateFeedList2(string feedXML)
        {
            // Load the feed into a SyndicationFeed instance
            StringReader stringReader = new StringReader(feedXML);
            XmlReader xmlReader = XmlReader.Create(stringReader);
            ...
            {
                // Bind the list of SyndicationItems to our ListBox
                feedListBox2.ItemsSource = feed.Items;

                loadFeedButton2.Content = "Refresh Feed";
            });
        ...
        private void UpdateFeedList3(string feedXML)
        {
            ...
            {
                // Bind the list of SyndicationItems to our ListBox
                feedListBox3.ItemsSource = feed.Items;

                loadFeedButton3.Content = "Refresh Feed";
            });
